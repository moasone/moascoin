...
//    Litecoin and modified for MoasCoin.
//
//    Parameters: lookup (int) - How many blocks to look into the past.
//    Returns: Estimated MoasCoin network hash rate (uint64_t)
//
...
// WM - Implementation of getnetworkhashps for MoasCoin
Value getnetworkhashps(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "getnetworkhashps\n"
            "Returns an estimate of the MoasCoin network hash rate.");
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "MoasCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "MoasCoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "MoasCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "MoasCoin is downloading blocks...");
